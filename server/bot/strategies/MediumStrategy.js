// Medium Bot Strategy - Table manipulation + optimal placement
// (Formerly Hard strategy)
...
export class MediumStrategy {
  ...
  play(state) {
    ...
    // If no initial meld yet, delegate to easy strategy
    if (!hasPlayedInitialMeld) {
      return this.easyStrategy.play(state);
    }
  ...
  tryTakeFromGroup(playerTile, playerTiles, tableSets) {
    ...
    for (let setIdx = 0; setIdx < tableSets.length; setIdx++) {
      ...
      for (const tableTile of set) {
        ...
        // Try to form a run with the table tile
